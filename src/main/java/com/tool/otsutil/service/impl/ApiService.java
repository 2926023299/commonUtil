...
//            }
		...
		// 如果响应为空或数据为空，则重试一次
		if (response == null || ObjectUtil.hasNull(response) || response.getData() == null || response.getData().isEmpty()) {
			log.warn("接口{}首次请求无数据返回，尝试重试一次", url);
			try {
				// 等待短暂时间再重试
				Thread.sleep(1000);
				response = httpService.fetchData(url, typeReference);
				log.info("接口{}重试请求完成", url);
			} catch (Exception e) {
				log.error("接口{}重试请求失败", url, e);
			}
		}
